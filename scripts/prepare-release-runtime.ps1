...
$ffprobePath = Join-Path $root 'src-tauri\resources\runtime\ffmpeg\ffprobe.exe'
...
if (!(Test-Path -LiteralPath $ffprobePath)) {
  throw "Bundled FFprobe was not found at '$ffprobePath'. Place ffprobe.exe there before running npm.cmd run tauri:build."
}
